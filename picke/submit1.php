...
if (empty($_POST["group"])&&($flag==0)) {
    $response= "Group name is required";
    $flag = 1;
  } 
if (empty($_POST["city"])&&($flag==0)) {
    $response= "College Location is required";
    $flag = 1;
  } 
...
$sql="SELECT * FROM `regist` WHERE  group='$group'";
...
elseif ($trent=="1") {
$response="GROUP NAME ALREADY TAKEN";
  }
else
 {
...
$sch_id='0'.$sch_id;
 ...
 $roll='KA'.$sch_id.$spe.$reg_num;

}
else if($city=='KANNUR'){
   $roll='KN'.$sch_id.$spe.$reg_num;
}
else if($city=='KOTTAYAM'){
   $roll='KT'.$sch_id.$spe.$reg_num;
}
else if($city=='PALA'){
   $roll='PL'.$sch_id.$spe.$reg_num;
}
else if($city=='KANJIRAPALLY'){
   $roll='KP'.$sch_id.$spe.$reg_num;
}
else
{
     $roll=substr($city,0,2).$sch_id.$spe.$reg_num;}
     //echo $roll;
     $sql = "INSERT INTO regist (id, fullName, class, school,contact, email,roll_num, city,grouped)
	     VALUES (NULL, '$name','$class','$school','$contact','$email','$roll','$city','$group')";
     $result=$conn->query($sql);
